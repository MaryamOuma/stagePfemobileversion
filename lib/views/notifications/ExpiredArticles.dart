...
import 'package:flutter_project/widgets/ArticleCommandList.dart';

import 'package:get/get.dart';
import '../navigation_drawer.dart';

class ExpiredArticles extends GetView<NotificationController> {
            ...
            child: Obx(
              () {
                ...
                return ListView.builder(
                  ...
                  itemCount: data.expiredarticles?.length ?? 0,
                  itemBuilder: (context, index) {
                    final command = data.expiredarticles![index];

                    // print('Command at index $index: $command');
                    return ArticleCommandCard(
                      reference: command.reference,
                      managingType: command.managingType,
                      active: command.active,
                      threshold: command.threshold,
                      premptionDate: command.premptionDate,
                      name: command.name,
...
class _SearchBarState extends State<SearchBar>
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        if (!_isActive)
          Text("Articles", style: Theme.of(context).appBarTheme.titleTextStyle),
        Expanded(
          child: Align(
